...
def wg_up():
	...
	selected_server = request.form["vpn-custom"]
	wg_interface = None
	...
	if get_vpn_active():
		...
		# Check if connection is actually up: ping -c1 (one ping) -W2 (timeout 2 sec)
		ping_response = subprocess.run(["ping", "-c1", "-W2", "8.8.8.8"], stdout = subprocess.PIPE).stdout.decode()
		ping_successful = bool(re.search(r"\d+\sbytes\sfrom\s[\d.]+:\sicmp_seq=\d+\sttl=\d+\stime=[\d.]+\sms", ping_response))

		if not ping_successful:
			shutdown_response = subprocess.run(["wg-quick", "down", wg_interface], stderr = subprocess.PIPE).stderr.decode()
			action_response = "VPN was up, but network was not reachable:\n\n" + ping_response + "As a result, VPN was shut down:\n" + shutdown_response + "\n" + "Is the Mullvad account active? Check remaining days on account."

	return render_template("index.html", wan_status = get_wan_status(), wg_status = get_wg_status(), 
		action_response = action_response, server_list = server_list, 
		wg_interface = wg_interface, selected_server = selected_server)
...
def wg_down():
	...
	if wg_status:
		if wg_interface == app.config["VPN_INTERFACE_US"] or wg_interface == app.config["VPN_INTERFACE_UK"] or wg_interface == app.config["VPN_INTERFACE_CUSTOM"]:
			...
			action_response = "No US or UK VPN active, but another one: " + wg_status["interface"] 
	else:
